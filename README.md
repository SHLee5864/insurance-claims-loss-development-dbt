...
| INT | view | Reconstruction by AY · DevMonth · Valuation axes. Spine creation. Triangle inputs. |
| MART | table | Triangle, LDF, Ultimate Loss, Loss Ratio — ready for actuarial analysis |

> INT models are materialized as views for simplicity. In production with larger datasets, `int_claim_monthly_paid` would be the first candidate for table materialization due to its Claim × Month spine explosion.
